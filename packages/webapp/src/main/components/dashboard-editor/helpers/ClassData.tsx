...
export function getAttributeNamesByClassName(className: string): string[] {
  const currentProject = ProjectStorageRepository.getCurrentProject();
  const classDiagram = currentProject?.diagrams?.ClassDiagram?.model || null;

  if (!classDiagram || !classDiagram.elements) return [];
  // Find the class element by name
  const classElement = Object.values(classDiagram.elements)
    .find((element: any) => element.type === 'Class' && element.name === className);
  if (!classElement || !(classElement as any).id) return [];
  const classId = (classElement as any).id;
  // Find all attributes owned by this class
  return Object.values(classDiagram.elements)
